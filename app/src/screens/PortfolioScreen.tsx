    ...
    const renderItem = ({ item }) => (
        <View style={styles.card}>
            <View style={styles.row}>
                <Text style={styles.ipoName}>{item.ipo_name}</Text>
            ...
            <View style={styles.row}>
                ...
                <Text style={styles.detailText}>Invested: ₹{item.invested_amount}</Text>
            </View>
            {item.status === 'SOLD' && (
            ...
            <Modal visible={modalVisible} animationType="slide" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput style={styles.input} placeholder="IPO Name" value={ipoName} onChangeText={setIpoName} />
                        <TextInput style={styles.input} placeholder="Invested Amount (₹)" value={investedAmount} onChangeText={setInvestedAmount} keyboardType="numeric" />
                        ...
                        <View style={styles.statusRow}>
                            {['APPLIED', 'ALLOTTED', 'NOT_ALLOTTED'].map(s => (
                                <TouchableOpacity key={s} onPress={() => setStatus(s)} style={[styles.statusOption, status === s && styles.statusActive]}>
                        ...
                        <View style={styles.modalButtons}>
                            ...
                            <TouchableOpacity onPress={handleAddTransaction} style={styles.saveButton}><Text style={styles.saveText}>Save</Text></TouchableOpacity>
                        </View>
                    </View>
                </View>
            </Modal>
...
const styles = StyleSheet.create({
    ...
    topBar: { flexDirection: 'row', justifyContent: 'space-between', padding: theme.spacing.md, alignItems: 'center', backgroundColor: '#fff' },
    screenTitle: { ...theme.typography.header, fontSize: 20 },
    logoutText: { color: theme.colors.error, fontWeight: 'bold' },
    ...
    summaryValue: { color: '#fff', fontSize: 24, fontWeight: 'bold', marginBottom: theme.spacing.md },
    profitContainer: { marginTop: theme.spacing.xs, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)', paddingTop: theme.spacing.sm },
    profitValue: { fontSize: 18, fontWeight: 'bold' },

    // List Item
    card: { backgroundColor: '#fff', padding: theme.spacing.md, borderRadius: 8, marginBottom: theme.spacing.sm, ...theme.shadows.card },
    row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: theme.spacing.xs },
    ipoName: { fontSize: 16, fontWeight: 'bold', color: theme.colors.text },
    ...
    // FAB
    fab: { position: 'absolute', bottom: 20, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: theme.colors.secondary, alignItems: 'center', justifyContent: 'center', elevation: 5 },
    fabText: { color: '#fff', fontSize: 32, marginTop: -2 },

    // Modal
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 },
    modalContent: { backgroundColor: '#fff', borderRadius: 12, padding: 20 },
    modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 20, color: theme.colors.primary },
    ...
    statusOption: { padding: 8, borderWidth: 1, borderColor: theme.colors.primary, borderRadius: 20, marginRight: 8, marginBottom: 8 },
    statusActive: { backgroundColor: theme.colors.primary },
    statusText: { color: theme.colors.primary, fontSize: 12 },
});
